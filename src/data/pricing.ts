...
export const PRICING: PricingRow[] = [
  ...
  {"idx":30,"الم��اس":"6x3","المستوى":"A","الزبون":"مسوق","شهر واحد":"0","2 أشهر":3200,"3 أشهر":4000,"6 أشهر":6800,"سنة كاملة":13000,"يوم واحد":10,"id":31},
  ...
  {"idx":35,"المقاس":"6x3","المستوى":"B","الزبون":"شركات","شهر واحد":"0","2 أشهر":4500,"3 أش��ر":6500,"6 أشهر":9500,"سنة كاملة":17500,"يوم واحد":10,"id":36},
...
export function getDailyPriceFor(size: string, level: LevelType | undefined, customer: CustomerType): number | null {
  const cs = canonSize(size);
  const lvl = level || 'A';
  let row = PRICING.find(r => canonSize(r['المقاس']) === cs && r['المستوى'] === lvl && r['الزبون'] === customer);
  if (!row) row = PRICING.find(r => canonSize(r['المقاس']) === cs && r['الزبون'] === customer);
  if (!row) row = PRICING.find(r => canonSize(r['المقاس']) === cs);
  if (!row) return null;
  return normalizeNumber((row as any)['يوم واحد']);
}
